...
export default function ApiKeys() {
  const [data, setData] = useState<any>({ items: [], supported: EXCHANGES, configured: [] });
  const [editing, setEditing] = useState<any>(null); // exchange or null
  const [form, setForm] = useState({ api_key: "", api_secret: "", passphrase: "", label: "", permissions: ["read"] });
  const [status, setStatus] = useState<any>(null);
  const [tests, setTests] = useState<Record<string, any>>({}); // exchange -> {loading, ok, ms, balances}
