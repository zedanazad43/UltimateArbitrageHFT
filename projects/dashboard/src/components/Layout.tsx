...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const current: NavItem =
    visibleNav.find((n) => n.to === loc.pathname) ||
    NAV.find((n) => n.to === loc.pathname) ||
    NAV[0];
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const { data } = await api.get<WorkerHealth>("/worker/health");
        if (m) setWorker(data);
      } catch (err) {
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <div className="min-w-0">
              <div className="text-sm font-medium truncate">{(user && "name" in user && user.name) || "Admin"}</div>
              <div className="text-[11px] text-muted font-mono truncate">{(user && "email" in user && user.email) || "—"}</div>
            </div>
            <span
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        <header className="h-14 border-b border-border/60 bg-surface/40 backdrop-blur-sm flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <CurrentIcon size={16} className="text-muted hidden lg:block" />
