...
// ─── JSON response helpers (used by /rocket-verify, /hermes/*, /api/stats…) ──
function ok(c, data = {}) {
  return c.json({ ok: true, data, timestamp: new Date().toISOString() });
}

function err(c, code, status = 500, message = '') {
...
// NOTE: headers must be applied AFTER next() — assigning c.res before the
// handler runs snapshots the empty default body and breaks every response.
...
app.get('/status', async (c) => {
  ...
  const [state, _lastScanBotState, _lastScanTombstone, _lastScanKV, dbMetrics, dbTrades, liveBalances] = await Promise.all([
