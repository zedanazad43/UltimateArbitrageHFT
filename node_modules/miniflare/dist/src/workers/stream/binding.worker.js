...
async function getEntryUrl(env) {
  let url = await (await env.MINIFLARE_LOOPBACK.fetch(
...
var StreamBinding = class extends WorkerEntrypoint {
  ...
  async upload(urlOrBody, params) {
    ...
    if (typeof urlOrBody == "string") {
      ...
      body = urlOrBody;
    let row = await getStub(this.env).createVideo(body, params ?? {}), entryUrl = await getEntryUrl(this.env);
  ...
  async details() {
    let row = await getStub(this.#env).getVideo(this.id), entryUrl = await getEntryUrl(this.#env);
    return rowToStreamVideo(row, entryUrl);
  }
  async update(params) {
    let row = await getStub(this.#env).updateVideo(this.id, params), entryUrl = await getEntryUrl(this.#env);
    return rowToStreamVideo(row, entryUrl);
  }
  async delete() {
  ...
  async list(params) {
    let rows = await getStub(this.#env).listVideos(params), entryUrl = await getEntryUrl(this.#env);
