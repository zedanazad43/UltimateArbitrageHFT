...
// ── Admin: Clear stuck paper positions ───────────────────────────────────────
app.post('/admin/clear-paper-positions', async (c) => {
  const limited = await checkRateLimit(c.env, c);
  if (limited) return limited;
  if (!isAuthorized(c.env, c)) return authDenied(c.env, c);
  try {
    ...
    const result = await c.env.DB.prepare(
      `UPDATE paper_positions SET closed_at = ?, exit_price = entry_price, pnl_usd = 0 WHERE closed_at IS NULL`
    ).bind(nowMs).run();
    const count = result?.meta?.changes ?? result?.changes ?? 0;
    await logAdminEvent(c.env, 'clear-paper-positions', c.req.raw);
    await sendTelegramAlert(c.env, `🧹 تم إغلاق ${count} مركز ورقي معلق`);
    return c.json({ success: true, closed: count });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
});

// ── Admin: Scan status — show last scan results and execution errors ──────────
app.get('/api/scan-status', async (c) => {
  if (!isAuthorized(c.env, c)) return authDenied(c.env, c, true);
  try {
    const [lastScan, state] = await Promise.all([
      c.env.BOT_STATE.get('nexus_last_scan', 'json').catch(() => null),
      getState(c.env),
    ]);
    const configuredExchanges = getConfiguredExchanges(c.env);
    const openPositions = await c.env.DB.prepare(
      `SELECT id, symbol, strategy, entry_price, size_usd, created_at FROM paper_positions WHERE closed_at IS NULL`
    ).all().catch(() => ({ results: [] }));
    ...
    return c.json({
      ...
      lastScan,
      openPaperPositions: openPositions.results ?? [],
      note: configuredExchanges.length < 2
        ? 'CEX arb needs ≥2 configured exchanges. Set EXCHANGE_API_KEY+SECRET secrets.'
        : `CEX arb ready — ${configuredExchanges.length} exchanges configured`,
    });
  } catch (err) {
    return c.json({ success: false, error: err.message }, 500);
  }
...
// ── API: AI Analysis (dashboard panel) ────────────────────────────────────────
// POST /api/ai-analysis  { opportunity: { symbol, strategy, direction, buyPrice, sellPrice, netPct } }
...
app.post('/api/ai-analysis', async (c) => {
  ...
  if (!opp || !opp.symbol) return c.json({ error: 'Missing opportunity.symbol' }, 400);

  const prompt = `You are an expert crypto arbitrage analyst. Evaluate this opportunity and give a concise risk/reward assessment.

Symbol: ${opp.symbol}
Strategy: ${opp.strategy || 'N/A'}
Direction: ${opp.direction || 'N/A'}
Buy Price: ${opp.buyPrice}
Sell Price: ${opp.sellPrice}
Net Profit %: ${opp.netPct}%

Provide a brief analysis (3-5 sentences) covering: potential profitability, key risks, and a recommendation (EXECUTE / SKIP / MONITOR).`;

  const MODEL = '@cf/meta/llama-3.1-8b-instruct';
  try {
    const result = await c.env.AIWORKER.run(MODEL, {
      messages: [{ role: 'user', content: prompt }],
      max_tokens: 400,
    });
    const analysis = result?.response ?? result?.text ?? JSON.stringify(result);
    return c.json({ provider: 'cloudflare-workers-ai', analysis });
  } catch (e) {
...
// --- legacy DO stub: keeps Cloudflare migration tracker happy, never instantiated ---
export class HFTBackup {
  constructor(state, env) { this.state = state; this.env = env; }
  async fetch(request) { return new Response('HFTBackup is deprecated', { status: 410 }); }
}
