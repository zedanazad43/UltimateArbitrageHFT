...
/** Fire-and-forget Telegram alert used inside executeTrade for leg-risk exposure. */
async function notifyTelegram(env, message) {
  ...
  try {
    ...
    // Properly cancel response to avoid stalled response warnings
    if (res.body) await res.body.cancel();
  } catch (_e) {
    // Silent fail for fire-and-forget alerts
  }
}
...
async function executeTrade(env, opp, sizeUsd, leverage) {
  ...
  // Use allSettled so a single-leg failure triggers an emergency alert instead
  // of silently leaving a one-sided open position (leg risk).
  ...
  // Both legs failed — clean failure, no exposure
  if (buyResult.status === 'rejected' && sellResult.status === 'rejected') {
    throw new Error(
      `Both legs failed — BUY(${buyExch}): ${buyResult.reason?.message} | ` +
      `SELL(${sellExch}): ${sellResult.reason?.message}`
    );
  }

  // Sell succeeded but buy failed — we sold without buying (short exposure)
  if (buyResult.status === 'rejected') {
    const msg =
      `🚨 [تعرض الساق] SELL ${opp.symbol} نجح على ${sellExch} لكن BUY على ${buyExch} فشل!\n` +
      `السبب: ${buyResult.reason?.message}\n` +
      `⚠️ يجب شراء ${amount} ${baseAsset} على ${buyExch} يدويًا لإغلاق المركز`;
    console.error('[CEX Leg Risk]', msg);
    await notifyTelegram(env, msg);
    throw new Error(`Buy leg failed after sell executed: ${buyResult.reason?.message}`);
  }

  // Buy succeeded but sell failed — we hold an unhedged long position
  if (sellResult.status === 'rejected') {
    const msg =
      `🚨 [تعرض الساق] BUY ${opp.symbol} نجح على ${buyExch} لكن SELL على ${sellExch} فشل!\n` +
      `السبب: ${sellResult.reason?.message}\n` +
      `⚠️ يجب بيع ${amount} ${baseAsset} على ${sellExch} يدويًا لإغلاق المركز`;
    console.error('[CEX Leg Risk]', msg);
    await notifyTelegram(env, msg);
    throw new Error(`Sell leg failed after buy executed: ${sellResult.reason?.message}`);
  }
