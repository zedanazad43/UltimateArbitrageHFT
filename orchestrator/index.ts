...
export class UniversalOrchestrator {
  ...
  async execute(req: OrchestratorRequest): Promise<OrchestratorResponse> {
    ...
    try {
      ...
      if (
        !this.tokenManager.canFitRequest(estimatedTokens) &&
        (this.tokenManager.getStatus().used_percent > 90)
      ) {
        return {
          id,
          task: req.task,
          ...
          success: false,
          error: "Token budget exhausted (in headroom zone)",
        };
      }
      ...
      // Step 4: Execute on selected agent
      const response = await this.bridge.execute(decision.agent, req.task);
      ...
      return {
        ...
        latency_ms: Date.now() - startTime + response.latency_ms,
