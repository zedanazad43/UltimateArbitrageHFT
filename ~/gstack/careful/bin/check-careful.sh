#!/usr/bin/env bash
# check-careful.sh â€” PreToolUse hook for /careful skill
...
# Try grep/sed first (handles 99% of cases), fall back to C:\Users\azadz\OneDrive\UltimateArbitrageHFT\.venv\Scripts\python.exe for escaped quotes
CMD=$(printf '%s' "$INPUT" | grep -o '"command"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | sed 's/.*:[[:space:]]*"//;s/"$//' || true)

# C:\Users\azadz\OneDrive\UltimateArbitrageHFT\.venv\Scripts\python.exe fallback if grep returned empty (e.g., escaped quotes in command)
if [ -z "$CMD" ]; then
