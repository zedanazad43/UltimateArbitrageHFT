#!/usr/bin/env bash
# check-freeze.sh â€” PreToolUse hook for /freeze skill
...
# Try grep/sed first, fall back to C:\Users\azadz\OneDrive\UltimateArbitrageHFT\.venv\Scripts\python.exe for escaped quotes
FILE_PATH=$(printf '%s' "$INPUT" | grep -o '"file_path"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | sed 's/.*:[[:space:]]*"//;s/"$//' || true)

# C:\Users\azadz\OneDrive\UltimateArbitrageHFT\.venv\Scripts\python.exe fallback if grep returned empty
if [ -z "$FILE_PATH" ]; then
...
case "$FILE_PATH" in
  "${FREEZE_DIR}/"*|"${FREEZE_DIR}")
    # Inside freeze boundary â€” allow
    echo '{}'
    ;;
  *)
    # Outside freeze boundary â€” deny
    # Log hook fire event
